...
export function NowPlaying({
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Vinyl + track info - centered layout */}
      <div className="flex flex-col items-center gap-5">
        {/* Vinyl spinner with glow */}
        <div className="relative flex shrink-0 justify-center">
          {/* Ambient glow behind vinyl */}
          <div 
            className="absolute inset-0 rounded-full blur-2xl -z-10"
            style={{
              background: effectivePlaying 
                ? `radial-gradient(circle, oklch(0.82 0.18 80 / 0.15) 0%, transparent 70%)`
                : "transparent",
              transform: "scale(1.3)",
        ...
        <div className="flex flex-col items-center gap-3 text-center max-w-sm">
          <div className="flex items-start justify-center gap-2">
            <h2 className="font-sans text-2xl sm:text-3xl font-bold tracking-tight text-foreground text-balance leading-tight">
              {track.title}
            </h2>
          ...
          <p className="font-sans text-xs text-muted-foreground">
            Queued by{" "}
            <span className="font-semibold" style={{ color: "oklch(0.72 0.12 250)" }}>
              {track.submittedBy}
            </span>
          </p>
            {soundCloudUrl && (
              <a
                ...
                className="mt-3 inline-flex items-center gap-2 rounded-lg px-4 py-2 font-sans text-xs font-semibold transition-all hover:scale-105 active:scale-95"
                style={{
        ...
        {micActive && (
          <div
            className="flex items-center justify-center gap-2.5 rounded-xl px-4 py-2.5 max-w-xs"
            style={{
              background: "oklch(0.13 0.03 30 / 0.7)",
              border: "1px solid oklch(0.50 0.24 30 / 0.5)",
              boxShadow: "0 0 20px oklch(0.50 0.24 30 / 0.2)",
            }}
          >
            ...
            <Mic className="h-4 w-4 shrink-0" style={{ color: "oklch(0.68 0.22 30)" }} />
            <span
        ...
        {/* Source badge */}
        <span
          className="rounded-lg px-3 py-1 font-sans text-[10px] font-bold uppercase tracking-wider"
          style={{
        ...
        >
          <div
            ...
            style={{
              ...
              background: "linear-gradient(90deg, oklch(0.72 0.18 250), oklch(0.82 0.18 80))",
              boxShadow: "0 0 12px oklch(0.82 0.18 80 / 0.5), 0 0 24px oklch(0.82 0.18 80 / 0.2)",
            }}
          />
          {/* Playhead glow */}
          <div
            ...
            style={{
              ...
              boxShadow: "0 0 12px oklch(0.82 0.18 80 / 0.8), 0 0 24px oklch(0.82 0.18 80 / 0.4)",
              transition: "left 1s linear",
            }}
          />
        </div>
        <div className="flex justify-between">
          <span className="font-mono text-xs font-medium" style={{ color: "oklch(0.72 0.18 250)" }}>
      ...
      <div className="flex items-center justify-center gap-6">
        ...
        {isDJ && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center justify-center h-12 w-12 rounded-full transition-all hover:scale-105 active:scale-95 disabled:opacity-40"
              style={{
                background: "linear-gradient(135deg, oklch(0.82 0.18 80), oklch(0.72 0.20 80))",
                boxShadow: "0 0 20px oklch(0.82 0.18 80 / 0.3), 0 4px 12px oklch(0.10 0.01 280 / 0.4)",
              }}
              aria-label={effectivePlaying ? "Pause" : "Play"}
            >
              {effectivePlaying ? (
                <Pause className="h-5 w-5" style={{ color: "oklch(0.12 0.02 80)" }} />
              ) : (
                <Play className="h-5 w-5 ml-0.5" style={{ color: "oklch(0.12 0.02 80)" }} />
              )}
            </button>
            <button
              onClick={onSkip}
              className="flex items-center justify-center h-10 w-10 rounded-full transition-all hover:scale-105 active:scale-95"
              style={{
                background: "oklch(0.18 0.01 280)",
        ...
        <div 
          ...
          style={{
            ...
            border: "1px solid oklch(0.22 0.01 280 / 0.5)",
          }}
        >
          <button
            onClick={toggleMute}
          ...
          >
            {muted ? (
              ...
              <Volume2 className="h-4 w-4" style={{ color: "oklch(0.72 0.18 250)" }} />
            )}
          </button>
          <Slider
