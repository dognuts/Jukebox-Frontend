...
function RoomCardImpl({ room }: { room: Room }) {
  ...
  return (
    ...
    >
      {/* Full-card navigation overlay. The favorite/reminder buttons below
          must not be nested inside an anchor (invalid HTML), so the link is
          a sibling overlay and the buttons sit above it (z-40). */}
      <Link
        href={`/room/${room.slug}`}
        aria-label={`Open ${room.name}`}
        className="absolute inset-0 z-30 rounded-t-[2rem] rounded-b-xl"
      />
        ...
        <div className="relative overflow-hidden rounded-t-[calc(2rem-2px)] rounded-b-[calc(0.75rem-2px)]" style={{ background: "oklch(0.12 0.01 280)" }}>
          ...
          <div className="relative mx-4 mt-6 mb-3 overflow-hidden rounded-xl" style={{ border: "1px solid oklch(0.35 0.03 60 / 0.4)" }}>
            ...
            >
              ...
              {/* Top row: badges. Sits above the navigation overlay (z-40 >
                  z-30) so the favorite button is clickable; pointer-events-none
                  lets clicks on the non-interactive badges fall through to
                  the overlay link. */}
              <div className="pointer-events-none relative z-40 flex items-start justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="pointer-events-auto flex items-center justify-center rounded-full p-1 transition-all"
                    style={{
          ...
          {isUpcoming ? (
            // z-40 lifts the reminder button above the navigation overlay
            <div className="relative z-40 mx-4 mb-2">
              <button
